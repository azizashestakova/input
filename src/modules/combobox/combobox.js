...
export default class Combobox {
  ...
  render(data) {
    let html = '';
    for (const value of data) html += `<li class="combobox__list-item" data-id="${value.id}">${value.city}</li>`;
  ...
  search() {
    this.input.addEventListener('keyup', event => {
      ...
      const data = this.data.filter(element => element.city.indexOf(value) !== -1);
      this.render(data);
    });
  }
